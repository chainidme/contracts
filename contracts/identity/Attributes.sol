...
abstract contract Attributes is IAttributes {
    ...
    mapping(bytes32 => uint256) public attributeCommitment;

    // hash(to, attr_index) => Attribute
    mapping(bytes32 => Attribute) public attributes;
    mapping(address => uint256) public attrCount;
    ...
    function setAttribute(
        ...
        attrCount[to].add(1);
        Attribute storage new_attr = attributes[
            keccak256(abi.encodePacked(to, attrCount[to]))
        ];
        ...
        bytes32 commitment = keccak256(
            abi.encodePacked(msg.sender, abi.encodePacked(to, name, value))
        );
    ...
    function commitAttribute(bytes32 attr, uint256 validForSeconds)
        public
        virtual
        override
    {
        attributeCommitment[
            keccak256(abi.encodePacked(msg.sender, attr))
        ] = validForSeconds;
    }

    function revokeAttribute(bytes32 attr) external virtual override {
        attributeCommitment[
            keccak256(abi.encodePacked(msg.sender, attr))
        ] = block.timestamp;
    ...
    function verifyAttribute(
        ...
        string memory name,
        string memory value
    ) external view virtual override returns (bool result) {
        uint256 validity = attributeCommitment[
            keccak256(abi.encodePacked(from, abi.encodePacked(to, name, value)))
        ];
        return (validity < block.timestamp);
    }

    function getAttribute(address to, uint256 index)
        public
        view
        virtual
        override
        ...
        returns (Attribute[] memory)
    {
        ...
        for (uint256 i = 0; i < attrCount[recipient]; i.add(1)) {
