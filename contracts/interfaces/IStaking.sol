...
interface IStaking {
    ...
    function hasStakedBefore(address _peer, bytes32 _identity) external view returns(bool);
    function getStake(address _peer, bytes32 _identity) external view returns(uint);

    function hasAcknowledgedBefore(bytes32 _identity, address _peer) external view returns(bool);
